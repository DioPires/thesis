...
def get_data_from_robot():
  ...
  f = open('/home/diogopires/ros_workspace/catkin_ws/src/thesis/test_result_files/RFID/tag_detection_for_manual_checking.txt', 'r')
...
def get_data_from_manual_check():
  ...
  f = open('/home/diogopires/ros_workspace/catkin_ws/src/thesis/test_result_files/RFID/manual_tag_detection.txt', 'r')
...
def plots():
  ...
  time_diff = numpy.zeros(len(time_m_aux))
  for k in range(0, len(time_m_aux)):
    time_diff[k] = math.fabs((time_m[k] - time_r[k]) * 0.0000000001)
    
  mean_for_plot = numpy.zeros(len(time_m_aux))
  variance_for_plot = numpy.zeros(len(time_m_aux))
  norm_for_plot = numpy.zeros(len(time_m_aux))
  ...
  s1 = 'The mean of the time difference is ' + str(numpy.mean(time_diff))
  s2 = 'The variance of the time difference is ' + str(numpy.var(time_diff))
  print s1
  print s2
  ...
  plt.subplot(2, 2, 4)
  plt.plot(numpy.linspace(0, len(time_diff), len(time_diff), False), norm_for_plot)
  plt.title('Normal distribution of time_diff')
  plt.show()
